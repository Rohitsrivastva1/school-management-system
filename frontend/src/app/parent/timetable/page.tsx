...
import { timetableAPI, usersAPI } from '@/lib/api';
...
export default function ParentTimetablePage() {
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const response = await usersAPI.getStudents({ page: 1, limit: 100 });
      
      if (response.data.success) {
        // Filter students for this parent (in a real app, this would be filtered by parent relationship)
  ...
  const fetchTimetable = async (classId: string) => {
    try {
      const response = await timetableAPI.getTimetableByClass(classId);
              ...
              <div className="flex items-center space-x-2">
                ...
                {selectedStudent && (
                  <span className="text-blue-600">
                    - {selectedStudent.firstName}'s Class
                  </span>
                )}
              </div>
            </CardContent>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-semibold text-gray-900">
                      {new Set(Object.values(timetable).flat().map(e => e.teacherId)).size}
                    </p>
                  </div>
                </div>
              </CardContent>
